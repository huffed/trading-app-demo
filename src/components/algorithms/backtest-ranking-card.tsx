...
function RankingRow({ ticker, name, metrics, error, isExpanded, onToggle }: {
  ticker: string; name: string; metrics?: BacktestMetrics; error?: string;
  ...
  return (
    <div className="space-y-2">
      <button onClick={onToggle} className="flex items-center gap-2 py-2 w-full text-left cursor-pointer" type="button">
        ...
        <span className="text-xs tabular-nums text-muted-foreground w-12 text-right">{metrics.win_rate.toFixed(0)}% W</span>
        <span className="text-xs tabular-nums text-muted-foreground w-8 text-right">{metrics.total_trades}t</span>
      </button>
...
function RankingTable({ results, errors, tickers }: {
  results: Record<string, BacktestMetrics>; errors: Record<string, string>; tickers: TickerInfo[];
}) {
  const [expandedTicker, setExpandedTicker] = useState<string | null>(null);

  const ranked = tickers
    .filter((t) => results[t.ticker])
    .sort((a, b) => (results[b.ticker]?.total_return ?? 0) - (results[a.ticker]?.total_return ?? 0));
  ...
  return (
    <div className="divide-y">
      {ranked.map((t) => (
        <RankingRow key={t.ticker} ticker={t.ticker} name={t.name} metrics={results[t.ticker]}
          isExpanded={expandedTicker === t.ticker}
          onToggle={() => setExpandedTicker((p) => (p === t.ticker ? null : t.ticker))} />
      ))}
      {failed.map((t) => (
        <RankingRow key={t.ticker} ticker={t.ticker} name={t.name} error={errors[t.ticker]}
  ...
  async function handleBacktestAll() {
    ...
    for (const t of tickers) {
      try {
        ...
        if (result.success) setResults((prev) => ({ ...prev, [t.ticker]: result.data as BacktestMetrics }));
        else setErrors((prev) => ({ ...prev, [t.ticker]: result.error }));
      } catch { setErrors((prev) => ({ ...prev, [t.ticker]: "Backtest failed" })); }
      setProgress((p) => p + 1);
    }
    setIsRunning(false);
  }
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium flex items-center gap-2">
          <FlaskConical className="h-4 w-4" />Watchlist Backtest
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <Button variant="outline" size="sm" onClick={handleBacktestAll} disabled={isRunning} className="w-full">
          {isRunning ? <><Loader2 className="mr-2 h-3 w-3 animate-spin" />Backtesting... ({progress}/{tickers.length})</> : "Backtest Watchlist"}
        </Button>
