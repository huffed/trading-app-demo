...
function NumericField({ label, value, onChange, suffix }: { label: string; value: number; onChange: (v: number) => void; suffix?: string }) {
  return (
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-1">
          ...
          {draft.entry_conditions.length === 0 && <p className="text-xs text-muted-foreground">No entry conditions</p>}
        </div>
        <div className="space-y-1">
          ...
          {draft.exit_conditions.length === 0 && <p className="text-xs text-muted-foreground">No exit conditions</p>}
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          <NumericField label="Stop Loss" value={draft.stop_loss?.value ?? 5} onChange={(v) => updateRisk("stop_loss", v)} suffix="%" />
          <NumericField label="Take Profit" value={draft.take_profit?.value ?? 15} onChange={(v) => updateRisk("take_profit", v)} suffix="%" />
