  ...
  return (
    <div className="rounded-lg border p-3 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <RuleStatus
          label={`Daily loss: ${report.max_daily_loss}% max (${report.daily_loss_breaches} breaches)`}
          passed={report.daily_loss_breaches === 0}
        />
        <RuleStatus
          label={`Drawdown: ${report.peak_drawdown}%`}
          passed={!report.drawdown_breached}
        />
        <RuleStatus
          label={`Consecutive losses: ${report.max_consecutive_losses}`}
          passed={!report.kill_switch_triggered}
        />
        <RuleStatus
          label={`Consistency: ${report.worst_day_pct_of_profit.toFixed(0)}% max day`}
          passed={report.consistency_pass}
        />
      ...
      {(report.total_slippage > 0 || report.total_commission > 0) && (
        <div className="flex gap-4 text-xs text-muted-foreground">
          ...
          {report.total_commission > 0 && (
            <span>Commission: ${report.total_commission.toFixed(2)}</span>
          )}
        </div>
      )}
