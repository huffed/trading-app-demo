...
function PriceSection({ pos }: { pos: PaperPosition }) {
  const isOpen = pos.status === "open";
  const { data: quote, isLoading } = usePositionLiveQuote(pos.id, isOpen);
  return (
    <div className="space-y-1.5">
      ...
      {!isOpen && (
        <StatRow
          label="Close"
          value={formatPriceValue(pos.ticker, pos.broker_close_price ?? pos.exit_price ?? null)}
        />
      )}
      {isOpen && isLoading && !quote && (
        <div className="space-y-1.5">
...
function ProtectionSection({ pos }: { pos: PaperPosition }) {
  const slPips =
  ...
  return (
    <div className="space-y-1.5">
      ...
      <StatRow
        label="Net"
        value={
          <span className={`font-medium ${pnlColorClass(grossPnl)}`}>{formatPnl(grossPnl)}</span>
        }
      />
    </div>
  );
...
function TimeSection({ pos }: { pos: PaperPosition }) {
  const isOpen = pos.status === "open";
  ...
  return (
    <div className="space-y-1.5">
      ...
      <StatRow label="Open" value={fmt(pos.opened_at)} />
      {!isOpen && pos.closed_at && <StatRow label="Close" value={fmt(pos.closed_at)} />}
      <StatRow label="Duration" value={formatDuration(pos.opened_at, pos.closed_at)} />
    </div>
  );
}

function MaeMfeSection({ pos }: { pos: PaperPosition }) {
  const isLong = pos.side === "long";
  const { data: maeMfe, isLoading } = usePositionMaeMfe(pos.id, true);
  return (
      ...
      {maeMfe && (
        <div className="grid gap-x-6 gap-y-1.5 sm:grid-cols-2">
          <StatRow
            label="MAE"
            value={
              <span className="text-[var(--loss)]">-{maeMfe.mae_pips.toFixed(1)} pips</span>
            }
          ...
          <StatRow
            label="MFE"
            value={
              <span className="text-[var(--profit)]">+{maeMfe.mfe_pips.toFixed(1)} pips</span>
            }
...
export function PositionStatsPanel({ pos }: { pos: PaperPosition }) {
  return (
    <div className="grid gap-x-6 gap-y-3 px-4 py-3 sm:grid-cols-2">
      <PriceSection pos={pos} />
      <ProtectionSection pos={pos} />
      <ResultsSection pos={pos} />
      <TimeSection pos={pos} />
      <MaeMfeSection pos={pos} />
    </div>
  );
}
