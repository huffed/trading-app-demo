...
export function EngineActivityTab() {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          LLM decisions, gate refusals, and notable saves over the selected window. Same source of
          truth as <code>scripts/cohort-report.ts</code>.
        </p>
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <StatCard
        ...
        hint={
          data.llm_avg_confidence != null ? `avg confidence ${data.llm_avg_confidence}%` : undefined
        }
      />
      <StatCard
        icon={Activity}
        ...
        {saves.map((s, i) => (
          <div key={`${s.when}-${i}`} className="border-l-2 border-l-primary/40 pl-3 space-y-1">
            <div className="flex items-center gap-2 text-xs">
              <span className="text-muted-foreground tabular-nums">{s.when.slice(0, 16)}Z</span>
