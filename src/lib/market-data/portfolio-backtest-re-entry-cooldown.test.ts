 ...
 *
 *  2026-06-19: switched to a Date-incrementing generator. The prior
 *  `2026-01-${day++}` pattern emitted "2026-01-32T..." once day rolled
 *  past 31 — JS strict-ISO parsing rejects those, which the post-B.1.16
 *  invalid-date check now treats as a programmer error (throw). */
function makeRepeatLossCycle(): PriceBar[] {
  const bars: PriceBar[] = [];
  let price = 100;
  const cursor = new Date("2026-01-01T04:00:00Z");
  const next = (): string => {
...
describe("portfolio-backtest re-entry cooldown (Phase B.1.2)", () => {
  ...
  it("cooldown disabled passes through unchanged", () => {
    ...
    const result = runPortfolioBacktest(baseRules, prices, 10000, { reEntryCooldown: disabled });
  ...
  it("cooldown enabled (4h timeframe → 240min) refuses entries within 1 bar of loss exit", () => {
    ...
    const gated = runPortfolioBacktest(baseRules, prices, 10000, { reEntryCooldown: cooldown });
  ...
  it("explicit cooldown_minutes overrides timeframe-derived default", () => {
    ...
    const baseline = runPortfolioBacktest(baseRules, prices, 10000);
    const result = runPortfolioBacktest(baseRules, prices, 10000, { reEntryCooldown: longCooldown });
  ...
  it("zero-cooldown_minutes is effectively a no-op (any elapsed time > 0)", () => {
    ...
    const baseline = runPortfolioBacktest(baseRules, prices, 10000);
    const result = runPortfolioBacktest(baseRules, prices, 10000, { reEntryCooldown: noop });
    expect(result.trades.length).toBe(baseline.trades.length);
  });
});
