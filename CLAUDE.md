...
- **Current focus — MILESTONE M1: First Proven Stream (reframed 2026-07-20).** The deployed 4-algo gold portfolio is the pipeline's certification run, not the target-hitter: 30 demo trades (~2027-01) within ±30% of the E2.24.d fidelity-corrected baseline proves the whole search→validate→deploy→evidence chain. The old "stable gold library" milestone is retired — deterministic gold's fully-corrected ceiling is ~0.38–0.55%/mo at ML≤8 (one factor, redundant triggers; search space exhausted per E2.23). Returns scale by ADDING UNCORRELATED STREAMS after M1 (~0.4%/mo × √N): forex re-research + the pre-registered E2.26 LLM-trader trigger are the M2 levers. Gold-only rule unchanged in force until M1 (`[[feedback_gold_only_demo_stage]]`). See ROADMAP.md "MILESTONE M1".
...
- `quarterly-research-cycle-cron.sh` (quarterly 1st of Jan/Apr/Jul/Oct at 07:00 UTC) → `/api/cron/quarterly-cycle` (H.5 — generates 4-artifact research cycle report: feature library refresh + alpha library snapshot + decay report + new-hypothesis log template. Writes markdown to `/tmp/quanttrader-cycles/<cycle_id>-research-cycle.md` + returns full report in JSON response. Operator-owned archival: copy worth-keeping cycles into `scripts/canonical/cycles/` and commit. See `scripts/canonical/quarterly-research-cycle.md` for the process doc + operator review workflow.)
