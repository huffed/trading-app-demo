...
9. **Walk-forward re-fit (post-deploy, REV 2 + REV 4 per ROADMAP G.5)** — once the algo is active, the monthly walk-forward-opt cron (`scripts/walk-forward-opt-cron.sh` → `/api/cron/wfo` → `src/lib/algo-search/walk-forward-opt.ts`) re-runs the 96-variant Layer B sweep on a rolling 12-month window. When best-by-DSR differs from current AND `best_dsr > current_dsr + 0.05` AND the geometry actually differs, it UPDATEs `algorithms.rules` and writes a `wfo_rules_updated` audit event. **DRY_RUN=1 is the default** — first 2-3 monthly cycles confirm parameters don't flap before operator flips the crontab to `?dry_run=0`. This implements REV 4 (static → walk-forward-optimized deployment); REV 2 is the consequent demotion of Layer B (steps 4 + 5) to diagnostic-only / one-time exploration — production parameter mutation is the cron's job, not a re-run of the manual sweep.
