...
### E2.15 — Deterministic-rules refinement battery on top single-algo (~1 week build + ~30min compute) [FILED 2026-06-29 EVENING-FINAL post-LLM-veto + empirical daily_bias sweep]
...
- **Status:** AWAITING OPERATOR INPUT (this is the genuine operator-input ask — recalibrating one of the 5 constraints is the only mathematical path to "top profits" within otherwise stated constraints).
...
| 2026-06-29 EVENING-FINAL | **OPERATOR VETOED LLM-TRADER (no budget) → deterministic-only path required.** Ran daily_bias D1-trend-filter augmentation sweep across top 11 FTMO-passers. Empirical winner: `ARB rr3_lb3_r06_rf1_af0 + daily_bias_bullish` (logic=all) at risk_per_trade=0.88% → Sharpe 0.286, DD 5.68%→5.0% at scaled risk, WR 39.5%, 157 trades, **~0.63%/mo**. 3.3x better than prior baseline (0.20%/mo) but still 3-5x below 2-3% target. Filed **E2.15 (deterministic refinement battery: L1 D1-trend done; L2 session filter / L3 news-veto / L4 partial-exits / L5 OR-confluence remaining)** with expected stacked ceiling ~1%/mo at 5% DD. Filed **E2.16 (operator constraint recalibration — choose 1 of: lift DD gate to 10%, allow forex/indices, allow intraday + paid data, accept 0.5-1%/mo, re-allow LLM when budget allows)** as genuine operator decision. Honest math per academic literature (Faber 2007, Hurst-Ooi-Pedersen AQR 2014, Lo-MacKinlay survey): **2-3%/mo at <5% DD on single-asset single-timeframe deterministic-rules is mathematically improbable per established empirical record**. E2.13 + E2.14 marked VETOED. | daily-bias sweep output (this turn) + ROADMAP E2.15 + E2.16 entries |
