...
| E2.25.i residual (F1 + advisors) | OPERATOR + me | soon | **OPERATOR: disable public signup in Supabase Auth settings** (the env scope stops execution but signups still create accounts/read anon key) + decide on the `callumh047` account. ME ($0): revoke EXECUTE from anon on `handle_new_user`/`rls_auto_enable` SECURITY DEFINER fns; 3× `SET search_path=''`; 2 FK indexes (`activity_log.position_id`, `llm_decisions.user_id`); CRON_SECRET out of curl argv (F3); `getExchangeRate` auth (F4); middleware `/portfolios` matcher (F5). |
| Alpha-decay liveness monitor | me | any time ($0 + 1 migration) | The G.4 auto-pause cron has no dead-man signal (died silently ≥5d in the outage). Approach: route emits a per-run heartbeat (cron_idle tagged `cron='alpha_decay'` on success) → new `last_alpha_decay_tick()` SECURITY DEFINER RPC → dead-man job asserts <26h. |
