...
**E2.25.a — LIVE 4h PRICE CACHE CORRUPTED (P0, live money-path, ACTIVE).** A transient OANDA 401 at 2026-07-17T14:45Z (key is valid — 200 against api-fxpractice on retest; transient upstream hiccup) triggered the Twelve Data fallback, whose payload merged into the `XAU/USD 4h full` cache row via unguarded "newer-wins" write. Live DB now: 20,776 bars, **5,000 zero-volume, 310 Saturday bars, forming 13:00Z tail bar**; real OANDA data gone for all instants since ~2026-04-02. The 4 live algos scan this for primary-TF patterns + ATR gates + SL geometry NOW (the `1day`/daily_bias row is clean, on OANDA truth). Pinned files immune (sha-refuse). Remediation sequence: (1) add write-quality guard (reject zero-volume / weekend-stamped / forming / non-canonical-decimal bars on OANDA-class writes) — the systemic fix, prevents recurrence; (2) rebuild the 4h row from pinned (`rebuild-price-row-from-pinned.ts`) then let the next OANDA-healthy scan append the clean tail; (3) investigate the 401 (rate-limit? token validation blip?) + add fallback-provenance logging. OPERATOR-GATED (rewrites live money-path data). Root file sites: `prices.ts` fallback merge, `price-cache.ts` write path.
...
- **Status:** E2.25.a P0 ACTIVE (operator-gated live-data remediation). E2.25.b/c/d/f/g/h are code fixes queued behind E2.24.d per the strategy sequencing. E2.25.i needs a security re-run. E2.25.j feeds the resize decision.
