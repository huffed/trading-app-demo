...
- **✅ SPEC LOCKED 2026-07-29: `scripts/canonical/algo-search-2026-10.spec.md`** — N=1,696 cells (4 instruments × {30m,1h,4h} × 17 patterns × direction × bias-axis × session-axis-on-intraday), WR-35 two-level bars, time-relative Layer-B lookbacks, B=20,000 bootstrap with the mandated p-floor<α/N startup assert (2.50e-5 < 2.95e-5 ✓, arithmetic verified), sibling-aware composer with blended-WR≥35 hard reject, pre-registered frontier-closure kill criteria. IMMUTABLE from this commit. **Remaining before the run:** §7 machinery checklist (session_filter rule, bias+session axes in the enumerator, time-relative lookbacks, blended-WR gate, driver asserts — all buildable now, no selection touched) + pin refresh at run time.
