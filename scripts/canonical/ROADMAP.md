...
- **Gate → outputs:** final worst-window ML → final uniform risk (size to ML≤8; expect ~0.55–0.62%, possibly <0.58% once floating-equity raises ML) → populate `preregistration.json` for the deployed set + G.8 rewrite (numbers locked, tolerance ±30 vs ±50 resolved, FAIL action pre-committed).
- **Status:** READY (spec complete; ~1–2 days build + ~2–3h compute; $0). The interim 0.58% resize holds until this lands.
...
- **Status:** READY (folds into the E2.24.d re-derivation). Until then the 8.05% is a documented noise floor to net out — both conventions are leak-free as-implemented; the divergence is convention, not future data.
