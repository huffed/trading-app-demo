...
| ~~E2.25.i residual~~ | ✅ 2026-07-30 | done | **Public signup: operator DECLINED disabling it (2026-07-20) — do not re-propose.** Money path is already protected by the `OPERATOR_USER_ID` scope (foreign algos never execute). Remaining residual = shared provider-quota exposure via authenticated actions (backtest/chart/`getExchangeRate`); the cheap mitigation is F4 (auth `getExchangeRate`) + a quota cap, filed here not as a signup issue. **ALL SHIPPED 2026-07-30:** migration 00051 (revokes on handle_new_user/rls_auto_enable — dead-man tick RPCs deliberately left anon; search_path pinned 'public' not '' to keep the signup trigger alive; 2 FK indexes) — verified: tick RPCs still 200 anon, revoked fns 404/401; F3 bearer→stdin across 9 cron scripts (live 200s); F4 getExchangeRate auth-gated; F5 /portfolios matcher. G.4.a also CLOSED same day: sharpe_ratio merged into all 5 live rows (0.1036–0.2506, alpha-decay convention) — cron now reports no_baseline:0/insufficient_data:5 (armed, engages as trades accrue). |
...
| 2026-08-27 | **E2.25.c-COMPLETION — live daily_bias ran on stale D1 (believed-fixed gap found live).** Operator status-review surfaced the XAU D1 cache row 3 sessions stale (Aug 21→27) despite E2.25.c's 26h refresh. Root cause: the fix landed in `getFreshPricesForScan` but the SCAN's daily loader (`loadDailyBarsForScan`) still read `getCachedPrices` under the 7-DAY daily TTL — the exact gap E2.25.c described, unwired at the one call site that feeds live daily_bias/regime/ADX. **Realized impact: zero** — the Aug 21 entries fired when the row was fresh, and bias was deep-bullish under both stale and fresh SMA20 all week; the risk class (bias lags up to 7d at regime TURNS — exactly where E2.25.b measured divergence concentrating) is what mattered. **Fix:** loader routed through the liveCron fresh path (26h margin, DQ.4-guarded persist); 2 contract tests updated + 1 added; 1909/1909, build/lint clean; live-verified post-deploy (D1 row rewrote through the Aug 26 session on the next scan). Evidence-clock: NO reset — data-freshness bug-fix restoring the validated harness semantics (fresh D1), same class as DQ repairs. Side observations from the same review: DXY loader fine (1h TTL, unused by deployed 5); XAU 1h row dormant-stale since June (nothing reads it); DQ.4 off-grid log noise (~2.2k ancient winter-phase bars re-dropped per merge — benign, dedupe-covered). | engine-process-ticker loader + tests |
