...
- **E2.25.a.ii — fallback in-memory granularity artifact OBSERVED LIVE (P2, filed 2026-07-21 from paper-only monitoring).** DQ.4 guards PERSISTENCE but the fallback payload still serves the caller in-memory (by design). On 2026-07-19 21:00Z → 07-20 20:00Z every intraday-ATR block read ATR ≈ 17 / threshold ≈ 19 — exactly half the clean-4h scale (ATR ~34 / ~35 before and after; √4 = 2× is the 1h→4h range scaling) → those scans almost certainly evaluated 1h-granularity (or otherwise finer/short-window) fallback bars AS the 4h series. No harm this time (gate blocked in both scales — conservative), but had a pattern fired on that data it would have been a wrong-granularity entry. Fix direction: scan path should assert bar-grid phase/spacing (median-spacing check, same predicate as DQ.3) on the served series before evaluation and skip-with-reason on mismatch — turning silent wrong-granularity evaluation into an explicit `signal_no_action`. Pairs with E2.25.a.i provenance logging (know WHICH provider served). Evidence: activity_log signal_no_action reasons 07-19→07-21.
...
| E2.19 b–e/g | me | before WFO arm | D1 anchor semantics; Yahoo partial-bar guard; forex pinned rebuilds; pinned-loader port (validate-algo + revalidate-candidates off live price_cache — also E2.25.g.2); name-match nit |
| Stage 5.3 challenge-readiness packet | me | at ~20 paper trades | Swing account re-verification at purchase; challenge size re-derivation with run-until-target P(pass); fee arithmetic; Mac-vs-host decision. **+ REQUIRED (E2.29): a short LIVE-account run (real fills, ≥10 trades) before the real challenge — paper-only proved the config, NOT real execution (slippage/spread/gaps). This is the deferred execution-validation step; needs a live account created then.** |
...
| Alpha-decay liveness check | me | with E2.24.c batch | Dead-man watches scan/manage only; decay cron has no monitor (E2.25.i finding) |
