...
- **Status:** READY — natural slot is WITH the sizing-fork decision (its output is the decision's input); if the operator picks the fork on E2.24.d numbers alone, E2.27 becomes the first post-decision refinement.
...
|---|---|---|---|
| **Sizing/composition decision** | OPERATOR | now | The one genuine fork: 4-algo @0.47% (0.38%/mo) vs 5-algo +Engulfing @0.42% (0.55%/mo, +25% evidence cadence, needs MtM-ρ check + zombie protocol) vs hold interim 0.58% (FTMO-safe, above ≤8 band). Evidence: `e2-results/e2.24d-rederivation-2026-07-17.md` |
...
| E2.25.i security re-run | me | next session | Route-auth census + secrets scan + quota math (agent died on API overload; top findings already filed) |
...
| E2.24.c order-lifecycle | me | before Stage 5.3 | Post-POST broker query before voiding; close-retry queue; reconcile-cron units fix + scheduling |
| E2.24.h tail | me | any time ($0) | brokers/ sizing tests (`notionalToLots` never executed); stressTest one-shot-script deprecation headers; stagnant-exit `parseBarDate`; risk-pool account_capital |
...
| Stage 5.3 challenge-readiness packet | me | at ~20 demo trades | Swing account re-verification at purchase; challenge size re-derivation with run-until-target P(pass); fee arithmetic; Mac-vs-host decision |
