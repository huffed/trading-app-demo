...
### E2.30 — Research/deployment SPLIT: forex search pre-registration [FILED 2026-07-29, operator-directed "trading everything solid and available is the key to winning"]
...
- **Deliverables (all $0, pinned data, at the E2.30 sanctioned round):** (i) intraday cells WITHOUT daily_bias (bias becomes a search axis, per our own rule); (ii) session/killzone axis; (iii) time-relative lookbacks; (iv) a CONSCIOUS operator re-decision on the WR≥37 floor — keep (streak tolerance + consec-loss-halt interaction rationale) or add a pre-registered expectancy-based alternate gate; (v) pre-registered regime-conditional short methodology (symmetric design, not June-motivated).
