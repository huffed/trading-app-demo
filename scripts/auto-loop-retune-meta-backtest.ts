...
async function runOneAlgo(
  ...
  const variantAt = (
    when: Date,
    sundayList: Array<{ date: Date; variant: string }>,
    fallback: string
  ): string => {
    let v = fallback;
    for (const s of sundayList) {
  ...
  // RETUNE: trades from the variant the drift-triggered selector picked.
  const autoLoopTrades: BacktestTrade[] = [];
  for (const [key, trades] of tradesByVariant) {
    for (const t of trades) {
      if (variantAt(new Date(t.entry_date), sundayActive, spec.defaultVariantKey) === key) {
  ...
  // RANDOM CONTROL: re-walk Sundays, pick uniformly at random from the
  // 144 variants. Deterministic seed so the run is reproducible.
  // Seeded by spec.key so every algo gets the same selection sequence
  // structure but different specific picks.
  let seed = 0;
  for (let i = 0; i < spec.key.length; i++) seed = (seed * 31 + spec.key.charCodeAt(i)) >>> 0;
  const rand = () => {
    seed = (seed * 1664525 + 1013904223) >>> 0;
    return seed / 0x100000000;
  };
  const variantKeys = Array.from(tradesByVariant.keys());
  const randomActive: Array<{ date: Date; variant: string }> = [];
  let randomCurrent = spec.defaultVariantKey;
  for (const boundary of sundays) {
    const recentStart = boundary.getTime() - RECENT_WINDOW_DAYS * DAY_MS;
    const recentStates = statesInRange(inputs, recentStart, boundary.getTime());
    const recent = buildDistribution(recentStates);
    const driftV = detectDrift({ baseline, recent });
    if (driftV.flagged) {
      randomCurrent = variantKeys[Math.floor(rand() * variantKeys.length)];
    }
    randomActive.push({ date: boundary, variant: randomCurrent });
  }
  const randomTrades: BacktestTrade[] = [];
  for (const [key, trades] of tradesByVariant) {
    for (const t of trades) {
      if (variantAt(new Date(t.entry_date), randomActive, spec.defaultVariantKey) === key) {
        randomTrades.push(t);
      }
    }
  }
  randomTrades.sort((a, b) =>
    new Date(a.exit_date).getTime() - new Date(b.exit_date).getTime()
  );

  // ORACLE: hindsight ceiling. The single best fixed variant across the
  // full corpus — what a perfect train-period optimizer would have
  // produced. Retune should be measured against this ceiling, not just
  // against the live default.
  let oracleKey = spec.defaultVariantKey;
  let oracleReturn = -Infinity;
  for (const [key, trades] of tradesByVariant) {
  ...
  const oracleTrades = tradesByVariant.get(oracleKey) ?? [];
  ...
  console.log(
    `  RETUNE (drift-triggered selector):                   total $${auto.totalReturn.toFixed(0).padStart(7)}  DD ${auto.maxDrawdownPct.toFixed(2)}%  ${auto.trades} tr  ${auto.winRate.toFixed(0)}% WR`
  );
  console.log(
    `  RANDOM (same trigger, uniform pick):                 total $${randomStats.totalReturn.toFixed(0).padStart(7)}  DD ${randomStats.maxDrawdownPct.toFixed(2)}%  ${randomStats.trades} tr  ${randomStats.winRate.toFixed(0)}% WR`
  );
  console.log(
    `  ORACLE (best fixed variant, hindsight) [${oracleKey.padEnd(22)}]: total $${oracle.totalReturn.toFixed(0).padStart(7)}  DD ${oracle.maxDrawdownPct.toFixed(2)}%  ${oracle.trades} tr  ${oracle.winRate.toFixed(0)}% WR`
  );
  const retuneVsRandom = auto.totalReturn - randomStats.totalReturn;
  const retuneVsStatic = auto.totalReturn - stat.totalReturn;
  console.log(
    `  Δ retune−static $${retuneVsStatic.toFixed(0)} · retune−random $${retuneVsRandom.toFixed(0)} · oracle ceiling $${oracle.totalReturn.toFixed(0)}` +
      `  (${retuneEvents.length} retune events on ${sundays.length} sundays)`
  );
  ...
  return {
    ...
    static: stat,
    auto_loop: auto,
    random_control: randomStats,
    oracle,
    oracle_variant: oracleKey,
