/**
 * Verify the algos affected by the liquidity_sweep bug fix (2026-06-18).
 * Runs RR×lb grid for each; reports top-return cell among FTMO-safe
 * (positive + static DD < 10%) AND among FTMO-safe + WR ≥ 37%.
 *
 * Affected algos all use liquidity_sweep either directly (Dip-Buyer) or
 * via liquidity_sweep_reclaim (sweep_reclaim-DailyBias variants).
 *
 * Question: did the pre-fix look-ahead inflate OR deflate their numbers?
 * If now-passing, we wrongly paused them. If still failing, the pause
 * decision stands.
 */
...
async function verifyAlgo(supabase: any, name: string): Promise<void> {
  ...
  if (!bars) { console.log(`  ${name}: no bars for ${ticker}`); return; }
  ...
  if (wrFiltered.length > 0) {
    ...
    console.log(`  TOP RETURN (FTMO-safe, WR ≥ 37%):   none — would stay paused`);
  }
}

async function main(): Promise<void> {
  console.log(`\n===== Sweep/Dip-Buyer affected algos verification @ ${new Date().toISOString().slice(0, 16)} =====`);
  console.log(`(Re-validated after fvg + liquidity_sweep bug fixes 2026-06-18)`);
