...
/** Wilder-smoothed ATR — inlined to avoid circular deps with the
 *  single-algo harness's internal computeAtr. Used for the adaptive
 *  TP cap (≤ 1.5 × daily ATR) per algo entry. */
function computeDailyAtrInline(bars: PriceBar[], period: number): number {
...
async function processAlgoAtBar(
  ...
  if ((decision.decision === "enter_long" || decision.decision === "enter_short") && !currentPos) {
    ...
    // RANGING regime block — mirror src/lib/scan/entry.ts gate.
    // 0/4 historical WR for RANGING entries on beyr1223h.
    ...
    // Adaptive TP context — regime-aware base RR + ATR cap. Mirror
    // src/lib/algorithm/structural-sl.ts:AdaptiveTpContext. Without this,
    // chop-day rr_multiple TPs land outside the day's likely range.
    const dailyBefore = corpus.dailyBars.filter(
      (d) => new Date(d.date).getTime() <= new Date(bar.date).getTime()
    );
    const dailyAtr =
...
async function main(): Promise<void> {
  ...
  // Optional per-algo prompt overrides via env. Lets us A/B test prompt
  // versions without editing DEFAULT_CONFIGS. Useful for v3 vs v4
  // comparison runs across multiple historical windows.
  const intradayPromptOverride = process.env.INTRADAY_PROMPT_VERSION as PromptVersion | undefined;
  const v1PromptOverride = process.env.V1_PROMPT_VERSION as PromptVersion | undefined;
  const configs: AlgoConfig[] = DEFAULT_CONFIGS.map((c) => {
    if (c.algoId === "intraday_30m" && intradayPromptOverride) {
      return { ...c, promptVersion: intradayPromptOverride };
    }
    if (c.algoId === "v1_4h_swing" && v1PromptOverride) {
      return { ...c, promptVersion: v1PromptOverride };
    }
    return c;
  });
